...
class file_writer:
    ...
    def __create_data_fields_of_record(self, field035, field008, references):
        ...
        data_fields += self.CONST_FIELD_SEPARATOR + field008
        directory += "008" + f'{len(field008):04}' + "00000"
        field035_res = self.CONST_FIELD_SEPARATOR + self.CONST_INDICATOR_1 + self.CONST_INDICATOR_2 + \
                       self.CONST_SUBFIELD_SEPARATOR + 'a' + field035 + self.CONST_FIELD_SEPARATOR
        data_fields += field035_res
        directory += "035" + f'{len(field035_res):04}' + f'{len(field008):05}'

        position = len(field008) + len(field035_res)
        for i in references:
            field = self.CONST_INDICATOR_1 + self.CONST_INDICATOR_2 + i.to_iso2709_string() + self.CONST_FIELD_SEPARATOR
            data_fields += field
            directory += "591" + f'{len(field):04}' + f'{position:05}'
            position += len(field)

        result = directory + data_fields
        return result
